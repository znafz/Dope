...
class TwoWayTest: BaseStream {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        publishView!.showDebugInfo(Stream.getParameter("debug_view") as! Bool)
    ...
    func subscribeBegin() {
        if( subscribeStream == nil ) {
            ...
            self.subscribeStream!.play(Stream.getParameter("stream2") as! String)
        }
    }
    ...
    override func onR5StreamStatus(stream: R5Stream!, withStatus statusCode: Int32, withMessage msg: String!) {
        ...
        if(stream == self.publishStream){
            ...
            if(Int(statusCode) == Int(r5_status_start_streaming.rawValue)){
                ...
                self.timer = NSTimer.scheduledTimerWithTimeInterval(1, target: self, selector: #selector(TwoWayTest.getStreams), userInfo: nil, repeats: false)
            }
        }
    }
    ...
    func onGetLiveStreams (streams : String){
        ...
        for i in 0..<names.count {
            ...
            if( Stream.getParameter("stream2") as! String == names[i] as! String )
            {
                subscribeBegin()
                return
            }
